...
if re.search(r"Sigma|Aldrich|Merck", text) :
...
elif re.search(r"Fisher", text):

    name = re.findall(r"Product Name.*?Cat No. :", text, re.DOTALL)[0].split('\n')
    name = [i.strip().strip(':').strip() for i in name if not re.search(r"Product Name|Cat No. :" , i) and re.search(r"\w", i)][0]
    print(name)
    ...
    print(cas)
    try:
        signalword = re.findall(r"Label Elements.*?Warning|Label Elements.*?Danger", text, re.DOTALL)[0].split()[-1]
    except:
        signalword = ''
